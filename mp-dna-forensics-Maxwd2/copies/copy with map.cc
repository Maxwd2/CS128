  ...
  for (unsigned int i = 1; i < data.size(); i++) {
    ...
    for (unsigned int j = 1; j < data.at(i).size(); j++) {
      ...
      if (num == map[str_names[j - 1]]) {
        count++;
        matches[data.at(i).at(0)] = count;
        std::cout << count << matches.at(data.at(i).at(0)) << std::endl;
      }
    }
  }
  std::string the_name;
  for (unsigned int i = 0; i < str_names.size(); i++) {
    for (unsigned int j = 0; j < str_names.size(); j++)
      if (matches.at(data.at(i + 1).at(i + 1))) == matches.at() && matches.at() == str_names.size()) {
          return "No Match";
        }
  }
  return matches;
