...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < carver.GetHeight(); i++) {
    for (int j = 0; j < carver.GetWidth(); j++) {
      // std::cout << carver.GetEnergy(i, j) << " ";
      std::cout << carver.GetImage().GetPixel(i, j).GetRed() << ", "
                << carver.GetImage().GetPixel(i, j).GetGreen() << ", "
                << carver.GetImage().GetPixel(i, j).GetBlue() << "    ";
    }
    std::cout << std::endl;
  }
  */ std::cout << "Weakest Vert seam: ";

  int* seam1 = carver.GetVerticalSeam();
  for (int i = 0; i < carver.GetHeight(); i++) {
    // std::cout << seam1[i] << " ";
  }
  std::cout << std::endl << "Weakest Hori seam: ";

  int* seam2 = carver.GetHorizontalSeam();
